...
(function($) {
    ...
    function renderResults(results, $container, query) {
        ...
        if (products.length > 0) {
            ...
            html += '<div class="us-results-group-title">ðŸ›ï¸ Products (' + products.length + ')</div>';
        ...
        if (posts.length > 0) {
            html += '<div class="us-results-group">';
            html += '<div class="us-results-group-title">ðŸ“ Blog Posts (' + posts.length + ')</div>';
            html += '<div class="us-results-grid">';
    ...
    function renderResultItem(result) {
        ...
        if (result.type === 'product' && result.add_to_cart) {
            ...
            html += '<a href="' + escapeHtml(result.add_to_cart_url) + '" class="button us-add-to-cart" data-product-id="' + result.id + '" onclick="event.stopPropagation();">ðŸ›’ Add to Cart</a>';
            html += '</div>';
        }
    ...
    function getNoResultsHTML() {
        return '<div class="us-no-results">' +
               '<div class="us-no-results-icon">ðŸ”</div>' +
    ...
    function getErrorHTML() {
        return '<div class="us-no-results">' +
               '<div class="us-no-results-icon">âš ï¸</div>' +
